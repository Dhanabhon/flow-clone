...
fn eject_device(device_path: &str) -> Result<(), String> {
    ...
    // unplugged. It exits non-zero with "Volume failed to eject" when a volume is
    // busy (Spotlight, Finder previews). Trust the exit code: on success the disk
    // is safe to remove even though the device node can linger on enclosures that
    // stay enumerated. If a busy volume blocked the eject, force-unmount every
    // volume and try once more.
    if diskutil_eject_ok(device_path)? {
        return Ok(());
    }
...
/// Runs `diskutil eject` and reports whether it succeeded. The spawn itself
/// failing (diskutil missing) is the only hard error.
#[cfg(target_os = "macos")]
fn diskutil_eject_ok(device_path: &str) -> Result<bool, String> {
